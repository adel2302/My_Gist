class GistsController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @gist.save
        format.html { redirect_to "/", notice: 'Gist was successfully created.' }
        format.json { render :show, status: :created, location: @gist }
      else
        format.html { render :new }
        format.json { render json: @gists.errors, status: :unprocessable_entity }
      end
    end
  end
  ...
  def update
    respond_to do |format|
      if @gist.update(gist_params)
        format.html { redirect_to "/", notice: 'Gist was successfully updated.' }
  ...
  private
    ...
    def gist_params
      params.require(:gist).permit(:title, :content, :user_id)
    end
end
